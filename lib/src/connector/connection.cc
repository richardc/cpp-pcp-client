...
Connection::Connection(const std::string& server_url,
    ...
    try {
        ...
        endpoint_thread_.reset(new boost::thread(&WS_Client_Type::run, endpoint_.get()));
    } catch (...) {
...
void Connection::connect(int max_connect_attempts) {
    ...
    do {
        ...
        case(ConnectionStateValues::initialized):
            ...
            connect_();
            boost::this_thread::sleep_for(boost::chrono::milliseconds(CONNECTION_MIN_INTERVAL));
            break;
        ...
        case(ConnectionStateValues::closed):
            ...
            if (previous_c_s == ConnectionStateValues::closed) {
                ...
                // thundering herd
                boost::this_thread::sleep_for(boost::chrono::milliseconds(connection_backoff_ms_ + dist(engine)));
                connect_();
                boost::this_thread::sleep_for(boost::chrono::milliseconds(CONNECTION_MIN_INTERVAL));
...
void Connection::onClose(WS_Connection_Handle hdl) {
    connection_timings_.close = boost::chrono::high_resolution_clock::now();
...
void Connection::onPreTCPInit(WS_Connection_Handle hdl) {
    connection_timings_.tcp_pre_init = boost::chrono::high_resolution_clock::now();
    LOG_TRACE("WebSocket pre-TCP initialization event");
}

void Connection::onPostTCPInit(WS_Connection_Handle hdl) {
    connection_timings_.tcp_post_init = boost::chrono::high_resolution_clock::now();
    LOG_TRACE("WebSocket post-TCP initialization event");
}

void Connection::onOpen(WS_Connection_Handle hdl) {
    connection_timings_.open = boost::chrono::high_resolution_clock::now();
